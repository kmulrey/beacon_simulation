...
/*
LORAPrimaryGeneratorAction::LORAPrimaryGeneratorAction(bool randomdirs, G4double energy=4.0*CLHEP::GeV)
{
   G4int n_particle = 1;
   particleGun = new G4ParticleGun(n_particle);
...
LORAPrimaryGeneratorAction::LORAPrimaryGeneratorAction(G4String pname, G4double zenith=0, G4double azimuth=0, G4double energy=4.0*CLHEP::GeV)
{
   G4int n_particle = 1;
   particleGun = new G4ParticleGun(n_particle);
...
LORAPrimaryGeneratorAction::LORAPrimaryGeneratorAction(G4String pname, G4double zenith=0, G4double azimuth=0, G4double energy=4.0*GeV, double xground=0, double yground=0)
{
   G4int n_particle = 1;
   particleGun = new G4ParticleGun(n_particle);
   ...
   G4ThreeVector loc(xground+sin(zenith)*cos(azimuth)*100*CLHEP::cm,yground+sin(zenith)*sin(azimuth)*100*cm,(-75+cos(zenith)*100)*CLHEP::cm);
   G4ThreeVector v(-sin(zenith)*cos(azimuth),-sin(zenith)*sin(azimuth),-cos(zenith));
...
void LORAPrimaryGeneratorAction::SetNewParticle(G4String pname="mu-", G4double zenith=0, G4double azimuth=0, G4double energy=4.0*CLHEP::GeV)
{   
   G4ParticleTable* particleTable = G4ParticleTable::GetParticleTable();
   particleGun->SetParticleDefinition(particleTable->FindParticle(pname)); //mu-
   particleGun->SetParticleEnergy(energy);
