...
BEACONDetectorConstruction::BEACONDetectorConstruction()
...
G4VPhysicalVolume* BEACONDetectorConstruction::Construct()
{
   ...
   G4Element* C = new G4Element("Carbon",   "C", z=6., a= 12.01*CLHEP::g/CLHEP::mole);
	...
	//Aluminium
	G4Material* Al = new G4Material("Aluminium", z=13., a= 27.0*CLHEP::g/CLHEP::mole, density= 2.70*CLHEP::g/CLHEP::cm3);
   
	//Scintillator polyvinyl-toluene: CH2CH(C6H4CH3)n [= 9 C + 10 H ]

   G4Material* Scinti = new G4Material("Scintillator", density= 1.032*CLHEP::g/CLHEP::cm3, nel=2);
   ...
   //------------------------------------------------------ volumes
	...
	G4double world_length= 350*CLHEP::cm;
	G4double world_height= 150*CLHEP::cm;
	...
	G4double box_width = 98*CLHEP::cm;
	G4double box_height = 9*CLHEP::cm;
   G4double box_length = 125*CLHEP::cm;
	G4double box_thickness = 1*CLHEP::mm;
   ...
   cavity_phys = new G4PVPlacement(0, G4ThreeVector(0,0,0), cavity_log, "cavity", cover_log, false, 0);
	...
	G4double scintillator_length=47.5*CLHEP::cm;
   G4double scintillator_thickness=3*CLHEP::cm;
	G4double sep=1*CLHEP::cm;
